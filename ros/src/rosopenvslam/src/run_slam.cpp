...
void mono_tracking(const std::shared_ptr<openvslam::config>& cfg, const std::string& vocab_file_path,
    ...
    // create publisher
    auto pose_publisher = node->create_publisher<geometry_msgs::msg::PoseStamped>("poses", 10);

    // get map publisher to gain access to current poses
    // NOTE: returned matrix from feed_mono goes wild when tracking is lost
    ...
    image_transport::Subscriber sub = image_transport::create_subscription(
        node.get(), "image", [&](const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
            const auto tp_1 = std::chrono::steady_clock::now();
            const auto timestamp = std::chrono::duration_cast<std::chrono::duration<double>>(tp_1 - tp_0).count();

            // input the current frame and estimate the camera pose
            openvslam::Mat44_t pose_matrix = SLAM.feed_monocular_frame(cv_bridge::toCvShare(msg, "bgr8")->image, timestamp, mask);

            // get current camera pose from map_publisher 
            /// NOTE: will publish last known pose if lost 
            // openvslam::Mat44_t pose_matrix = map_publisher->get_current_cam_pose();
